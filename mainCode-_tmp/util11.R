...
LDM_test = function(otu.tab, metadata, n.rej.stop = 100, n.perm.max=5000){
...
LinDA_test = function(otu.tab, metadata){
  res.linda = linda(
    ...
    p.adj.method = "BH", 
    alpha = 0.05)
  summary.tab = res.linda$output$Abx
...
MaAsLin2_test = function(otu.tab, metadata, output = './tmp'){
  ...
  res.ma = Maaslin2(
    ...
    output = output, 
    #transform = "AST",
    #fixed_effects = c('Abx', 'age', 'DDRT', 'PREOP_ABX', 'BACTRIM'),
    fixed_effects = c('Abx', 'age'),
    random_effects = c('study'),
    #normalization = 'NONE',
    plot_heatmap = F,
    plot_scatter = F,
    standardize = FALSE)
  summary.tab = res.ma$results %>% filter(name == 'Abx') 
...
NBZIMM_test = function(otu.tab, metadata, method = 'nb'){
  ...
  if (method == 'nb'){
    fit = mms(y = otu.tab, 
              #fixed = ~ Abx + age + DDRT + PREOP_ABX + BACTRIM + stats::offset(log(N)), 
  ...
  if (method == 'zig_prop'){
    ...
    fit = mms(y = otu.tab.asin, 
              #fixed = ~  Abx + age + DDRT + PREOP_ABX + BACTRIM , 
              fixed = ~  Abx + age, 
              random = ~ 1 | study, 
              data = metadata,
              min.p = 0, 
...
cauchy_combine_test = function(zerorate, 
  ...
  #I need to change this later if write package, because we only clip taus<0.1 or >0.9
  ...
  if (method == 'equal_clip'){
    if (is.na(logit_pval)){
      w = c(0, 0, rep(1,(l-2))/(l-2), 0)
	  w = w / sum(w)
    } else{
      w = c( zerorate, 0, rep(1/(l-1), (l-2)), 0) 
	  w = w / sum(w)
    }
  }
  
  if (method == 'zerorate_equal'){
    w = rep(-1,length(pval))
	w[1] = zerorate
	w[2:length(w)] = rep( (1-zerorate) / (length(pval)-1), length(pval)-1 )
	w = w /sum(w)
  }
  ...
  stats_cauchy = sum(w*tan((0.5-pval)*pi),na.rm = F)
  pval_cauchy = 1 - pcauchy(stats_cauchy)
  return(pval_cauchy)

}

Truncated_Cauchy_Combination <-function(pvalues, w = NULL, delta = 0.01, ignore_logit=0){
	l = length(pvalues)
  if(is.null(w)){ 
	p.not.na = ifelse( is.na(pvalues),0,1 )
	w = p.not.na / sum(p.not.na)
  }
...
#ZINQL_fit = function(X1m,Z,study,y, ybin, taus){
#ZINQL_fit = function(y, meta, X, Z, random, taus, method='both', seed=2024){
