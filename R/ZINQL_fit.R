...
minP_combine_test = function(pval,
	...
	#We conduct eigenvalue decomposition and drop negative eigenvalues
...
Truncated_Cauchy_Combination_zerorate_equal <-function(zerorate, pvalues, delta = 0.01){
	l = length(pvalues)
	w = rep(-1,l)
	w[1] = zerorate
...
ZINQL_fit = function(y, formula, formula.logistics=NA, meta, C, taus=c(0.1, 0.25, 0.5, 0.75, 0.9), method='Both', n.positive.cut=5, seed=2024){
	...
	#This is the final output project
	result.full = list()
	#
	...
	#
	nonzero_idx = which(y > 0)
	n.positive = length(nonzero_idx)
	#
	N = length(X1m)
	#
	...
	for(v.tmp in colnames(Z1m)){
		if(v.tmp == 'intercept'){
			...
			feature.value.unique = unique(meta[nonzero_idx,v.tmp])
	...
	#
	x.quantile.unique = unique(meta[nonzero_idx,X])
	...
	#result for logistics regression
	...
	#
	#This is the case where we have no variables left for quantile regression and the user does not allow intercept
	if(rely.logistics==1){
	...
	#No non-zero ys, so we set logistics regression p-value as one
	if( min(ybin)>0 ) {
	...
	#We conduct eigenvalue decomposition for negative eigenvalues
	...
	#
	#if the quantile regression cannot be fit successfully. We fill in one for quantile regression
	...
	if(run.cauchy == 1){
		pval_dep_cauchy_trunc_zero_equal = Truncated_Cauchy_Combination_zerorate_equal(zerorate = zerorate, pvalues = c(logit_pval_LRT, pval_dep))
